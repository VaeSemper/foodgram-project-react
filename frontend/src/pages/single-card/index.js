...
const SingleCard = ({ loadItem, updateOrders }) => {
  ...
  return <Main>
    <Container>
      <MetaTags>
        <title>{name}</title>
        <meta name="description" content={`Foodgram Assistant - ${name}`} />
      ...
      <div className={styles['single-card']}>
        ...
        <div className={styles["single-card__info"]}>
          ...
          <div className={styles['single-card__buttons']}>
            ...
            {is_in_shopping_cart ? <><Icons.DoneIcon color="#4A61DD"/>Recipe added</> : <><Icons.PlusIcon /> Add to shopping cart</>}
            </Button>}
            ...
            >
              {author.is_subscribed ? 'Unsubscribe from the author' : 'Subscribes on author'}
            </Button>}
          </div>
